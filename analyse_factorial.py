...
def load_ledger(ledger: Path, runs_root: Path):
    ...
    for row in csv.DictReader(open(ledger)):
        ...
        cells[(sc, row["ordering"], row["planner"])].append(data)
    return cells
...
def main():
    ...
    print(f"\n{'Sc':<4}{'Ordering':<12}{'Planner':<10}{'n':<4}"
          f"{'Tour s':>16}{'Path m':>16}{'Cover':>9}{'ArUco':>9}{'Success':>9}")
    print("-" * 90)
    for sc in scenarios:
        for o in ORDERINGS:
            for p in PLANNERS:
                ...
                if not runs:
                    continue
                tm, tsd, n = agg([r["tour"] for r in runs])
                pm, psd, _ = agg([r["path"] for r in runs])
                totm, totsd, _ = agg([r["total"] for r in runs])
                nvp = runs[0]["n_vp"]
                cov = st.mean([r["vp"] / nvp for r in runs]) * 100
                aru = st.mean([r["aruco"] / nvp for r in runs]) * 100
                succ = st.mean([1.0 if r["complete"] else 0.0 for r in runs]) * 100
                rows.append(dict(scenario=sc, ordering=o, planner=p, n=n,
                                 tour_mean=round(tm, 1), tour_std=round(tsd, 1),
                                 total_mean=round(totm, 1), total_std=round(totsd, 1),
                                 path_mean=round(pm, 1), path_std=round(psd, 1),
                                 coverage_pct=round(cov, 1), aruco_pct=round(aru, 1),
                                 success_rate_pct=round(succ, 1)))
                print(f"{sc:<4}{ORD_LABEL[o]:<12}{PL_LABEL[p]:<10}{n:<4}"
                      f"{tm:>10.1f}+/-{tsd:<4.1f}{pm:>10.1f}+/-{psd:<4.1f}"
                      f"{cov:>8.0f}%{aru:>8.0f}%{succ:>8.0f}%")
    ...
    for sc in scenarios:
        for p in PLANNERS:
            ...
            if not a or not b:
                continue
            d_t = st.mean([r["tour"] for r in b]) - st.mean([r["tour"] for r in a])
            d_p = st.mean([r["path"] for r in b]) - st.mean([r["path"] for r in a])
            base_p = st.mean([r["path"] for r in a])
            pct = 100 * d_p / base_p if base_p else 0
    ...
    for sc in scenarios:
        for o in ORDERINGS:
            ...
            if not base:
                continue
            bt = st.mean([r["tour"] for r in base])
            bp = st.mean([r["path"] for r in base])
            bs = st.mean([1.0 if r["complete"] else 0.0 for r in base]) * 100
            for p in ["astar", "rrts"]:
                c = cells.get((sc, o, p), [])
                if not c:
                    continue
                d_t = st.mean([r["tour"] for r in c]) - bt
                d_p = st.mean([r["path"] for r in c]) - bp
                d_s = st.mean([1.0 if r["complete"] else 0.0 for r in c]) * 100 - bs
    ...
    # ---------------- coverage warnings ----------------
    thin = [(k, len(v)) for k, v in cells.items() if len(v) < 2]
    print(f"\n{'='*90}")
