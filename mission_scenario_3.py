#!/usr/bin/env python3
"""Simple mission for a single drone (instrumented + ordering-aware)."""
...
if __name__ == '__main__':
    ...
    # Always give the logger its own node to avoid subscription/type conflicts.
    ...
    try:
        ...
        if success:
            ...
            # Ordering fix: compute ordering using the actual live start pose after takeoff.
            ordering_start = (
                float(start_pose["x"]),
                float(start_pose["y"]),
                float(start_pose["z"]),
            ...
            mission_success = drone_run(uav, scenario, ordered_mission.ordered_ids, logger)
        ...
        # Return to pre-run hover pose before landing so subsequent runs are comparable.
        try:
