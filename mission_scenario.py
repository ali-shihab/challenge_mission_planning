    ...
    *,
    ...
    if expected_marker_id is not None and aruco_tracker is not None:
        ...
        if aruco_result["verified"]:
            ...
            # Visual indicator for video recording
            mid = aruco_result.get("marker_id", expected_marker_id)
            banner = f"\n{'='*60}\n  ✓ ArUco MARKER {mid} VERIFIED at viewpoint {viewpoint_id}\n{'='*60}"
            print(banner)
        else:
            aruco_result["reason"] = "marker_not_seen_in_time"
            print(f"  ✗ ArUco marker {expected_marker_id} NOT seen at viewpoint {viewpoint_id}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--obstacle_inflation_m', type=float, default=0.4,
    ...
    parser.add_argument('--goto_timeout_s', type=float, default=60.0,
                        help='Hard timeout per go-to command')
    parser.add_argument('--stuck_timeout_s', type=float, default=20.0,
    ...
    parser.add_argument('--dwell_s', type=float, default=0.2,
    ...
    # ArUco inspection
    parser.add_argument('--aruco_topic', type=str, default='/drone0/sensor_measurements/hd_camera/image_raw',
                        help='Marker detection topic (default: hd_camera image_raw for drone0)')
    ...
    parser.add_argument('--return_to_start', action='store_true', default=True,
                        help='Return to origin (0,0) after mission timer stops (default: True)')
    ...
    if args.aruco_topic:
        ...
        tmp_tracker.attach_ros(logger_node)  # returns None on success
    ...
    try:
        ...
        # Always return to world origin (0,0,1m) if the drone ever left the ground,
        # regardless of mission success, failure, keyboard interrupt, or exception.
        # This guarantees a clean start position for the next run.
        if start_pose is not None:
            try:
                origin_pose = {"x": 0.0, "y": 0.0, "z": TAKE_OFF_HEIGHT, "yaw": 0.0}
                logger.event("return_to_origin_start", {"origin": origin_pose, "reason": ended_reason})
                _ = drone_return_to_start(
                    drone_interface=uav,
                    start_pose=origin_pose,
                    logger=logger,
                    goto_timeout_s=60.0,
                ...
                sleep(1.0)
                logger.event("return_to_origin_done", {})
            except Exception as e:
                logger.event("return_to_origin_failed", {"type": type(e).__name__, "msg": str(e)})
