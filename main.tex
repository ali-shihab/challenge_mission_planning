...
\begin{tikzpicture}[
  font=\footnotesize,
  box/.style={draw, rounded corners=2pt, minimum height=8mm, minimum width=26mm,
              align=center, inner xsep=3pt, fill=white},
  off/.style={box, fill=black!4},
  sim/.style={box, fill=black!12},
  ar/.style={-{Latex[length=2mm]}, semithick}
]
% ---- offline planning chain (row 1) ----
\node[off]                       (yaml) {scenario YAML};
\node[off, right=8mm of yaml]    (grid) {occupancy grid\\{\scriptsize 0.5\,m, 0.4\,m infl.}};
\node[off, right=8mm of grid]    (tsp)  {NN + 2-opt\\ordering};
\node[off, right=8mm of tsp]     (loc)  {local planner\\{\scriptsize straight / A* / RRT*}};

% ---- simulator chain (row 2) ----
\node[sim, below=13mm of yaml]   (gen)  {world generator};
\node[sim, right=8mm of gen]     (gz)   {Ignition Gazebo\\{\scriptsize obstacles + ArUco}};
\node[sim, right=8mm of gz]      (as2)  {AS2 platform\\{\scriptsize + behaviours}};
\node[off, right=8mm of as2]     (goto) {\texttt{GoTo} waypoints};

% ---- runtime monitoring (row 3) ----
\node[box, below=13mm of gz]     (ver)  {pose check 10\,Hz\\{\scriptsize + ArUco detect}};
\node[box, right=8mm of ver]     (log)  {JSONL events\\{\scriptsize trajectory 50\,Hz}};

% Arrows on the background layer so they never paint over a box or its text.
...
% ---- stage labels, placed clear of the boxes ----
\node[anchor=east, font=\scriptsize\itshape, align=right]
      at ([xshift=-4mm]yaml.west) {offline\\(pre-flight)};
\node[anchor=east, font=\scriptsize\itshape, align=right]
      at ([xshift=-4mm]ver.west) {in flight};
\end{tikzpicture}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{report_figs/fig_paths_scenario2.png}\\[3pt]
  \includegraphics[width=\textwidth]{report_figs/fig_paths_scenario4.png}
  \caption{Median run per cell. (a,b) Scenario~2; (c,d) Scenario~4; file ordering then NN+2-opt. File-order tours cross themselves repeatedly.}
  \label{fig:paths}
\end{figure}
